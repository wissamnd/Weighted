...
middleware(Store<ReduxState> store, action, NextDispatcher next) {
  ...
  } else if (action is SetTargetAction){
    _handleSetTargetAction(action, store);
  }
  next(action);
...
_handleLogoutAction(Store<ReduxState> store, LogoutAction action) async {
...
_handleSetTargetAction(SetTargetAction action,Store<ReduxState> store) {
  _setTarget(action.target)
      .then((nil)=> store.dispatch(new OnTargetChangedAction(action.target)));
}
...
_handleInitAction(Store<ReduxState> store) {
  ...
  _loadTarget().then((target)=>store.dispatch(new OnTargetChangedAction(target)));
...
Future _setTarget(double target) async{
  SharedPreferences prefs = await SharedPreferences.getInstance();
  prefs.setDouble('target', target);
}
...
Future<double> _loadTarget()async{
  SharedPreferences prefs = await SharedPreferences.getInstance();
  return prefs.getDouble('target');
}